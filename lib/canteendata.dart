...
class Cantcomplain extends StatefulWidget {
  const Cantcomplain ({Key? key}) : super(key: key);

  @override
  State<Cantcomplain> createState() => _CantcomplainState();
}

class _CantcomplainState extends State<Cantcomplain> {
  List _Complains=[];
  ...
  fetchcomplains() async{
    ...
    setState(() {
      for(int i=0;i<qn.docs.length;i++){
        if(qn.docs[i]["type"]=="Canteen" || qn.docs[i]["type"]=="canteen") {
          _Complains.add({
